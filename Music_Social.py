...
to_rem = np.setdiff1d(users,users_train)
...
for usr1 in users:
   for usr2 in users[i:len(users)]:
     ...
     soc_graph.add_edge((user_nodes['n'][usr1]),(user_nodes['n'][usr2]),weight = (wt_tag + wt_listn)/2 )
     i = i +1  
...
for user in users_test:
  ...
  ################ start the random walk with restart
   ...
   top_1[user] = len(np.intersect1d(recom_artists[0:1],artist_usr)) 
   top_5[user] = len(np.intersect1d(recom_artists[0:6],artist_usr)) 
   top_10[user] = len(np.intersect1d(recom_artists[0:11],artist_usr)) 
   top_20[user] = len(np.intersect1d(recom_artists[0:20],artist_usr)) 
